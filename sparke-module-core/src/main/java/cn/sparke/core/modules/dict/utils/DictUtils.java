package cn.sparke.core.modules.dict.utils;


import cn.sparke.core.common.constants.ProjectProperties;
import cn.sparke.core.modules.cache.CacheFacade;
import cn.sparke.core.modules.dict.entity.SysDictEntity;
import cn.sparke.core.modules.dict.mapper.DictMapper;
...
public class DictUtils {
    ...
    @Bean
    public DictUtils init(DictMapper dictMapper, ProjectProperties constants) {
        DictUtils.dictMapper = dictMapper;
     ...
     * @param type  类型
     */
    public static SysDictEntity getDict(String label, String type) {
        if (StringUtils.isNotBlank(type) && StringUtils.isNotBlank(label)) {
            for (SysDictEntity dict : getDictList(type)) {
                if (type.equals(dict.getType()) && label.equals(dict.getLabel())) {
                    return dict;
    ...
    public static List<SysDictEntity> getDictList(String type) {
        Map<String, List<SysDictEntity>> dictMap = null;
        if (!"local".equals(projectProperties.cache)) {
            dictMap = CacheFacade.getObject(CACHE_DICT_MAP, new TypeReference<Map<String, List<SysDictEntity>>>() {
            });
        }
        if (dictMap == null) {
            dictMap = new HashMap<>();
            List<SysDictEntity> allList = dictMapper.findList(null);
            for (SysDictEntity dict : allList) {
                List<SysDictEntity> dictList = dictMap.get(dict.getType());
                if (dictList != null) {
                    dictList.add(dict);
                } else {
        ...
        List<SysDictEntity> dictList = dictMap.get(type);
        if (dictList == null) {
    ...
    public static List<String> getDicLabelList(String type) {
        List<SysDictEntity> dictList = DictUtils.getDictList(type);
